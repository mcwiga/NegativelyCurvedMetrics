...
\begin{frame}[c]{Codazzi Equation}
	\begin{kulblock}{Definition}
		The Codazzi equation associated to the fundamental pair $(I,II)$ with shape operator $S$ is given by
		$$\nabla_X SY - \nabla_Y SX = S[X,Y]$$
	\end{kulblock}
\end{frame}
...
\begin{frame}[c]{Arc Length Paramaterization}
	\begin{kulblock}{Proposition (\cite{firstarticle})}
		Let $(I,II)$ be a Codazzi pair on an oriented, simply connected surface of negative constant extrinsic curvature $K$. Then we can write $(I, II)$ as
		$$I =\begin{pmatrix}
				1           & \cos \omega \\
				\cos \omega & 1
			\end{pmatrix}, \quad II = \begin{pmatrix}
...
\begin{frame}[c]{Change of Coordinates}
	...
	\begin{itemize}
		\item If $\dfrac{\partial E}{\partial v} = \dfrac{\partial G}{\partial u} = 0$ then
...
\begin{frame}[c]{We have a global diffeomorphism!}
	\begin{kulblock}{Proposition (\cite{firstarticle})}
		The map $ (u,v) : \Sigma \to \mathbb{R}^2$ is a global diffeomorphism.
	\end{kulblock}
	\begin{proof}
...
\begin{frame}[c]{Hilberts Theorem Proof}
	\begin{proof}
		...
		$$\frac{\partial}{\partial u}\omega_{v} = -(K+c)\sin \omega$$
		where $-(K+ c) > 0$.
	\end{proof}
\end{frame}
...
\begin{frame}[c]{Non-constant negative curvature}
	\begin{kulblock}{Corollary (\cite{GALVEZ20151058})}
		...
		$$k_i \geq \epsilon > 0, \quad \text{for } \epsilon>0 $$
